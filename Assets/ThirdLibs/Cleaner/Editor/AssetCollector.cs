	...
	public class AssetCollector
	{
		...
		public void Collection ()
		{
			try {
				...
				// Find assets
				var files = Directory.GetFiles ("Assets/Game/PackagingResources/HeNanMJ", "*.*", SearchOption.AllDirectories)
					.Where (item => Path.GetExtension (item) != ".meta")
					.Where (item => Path.GetExtension (item) != ".js")
					.Where (item => Path.GetExtension (item) != ".dll")
					.Where (item => Path.GetExtension (item) != ".prefab")
					...
					.Where (item => Regex.IsMatch (item, "[\\/\\\\]Sound") == false);
		...
		{
			var resourcesFiles = Directory.GetFiles ("Assets/Game/PackagingResources/", "*.*", SearchOption.AllDirectories)
				// .Where (item => Regex.IsMatch (item, "[\\/\\\\]Resources[\\/\\\\]") == true)
					.Where (item => Path.GetExtension (item) != ".meta")
					.ToArray ();

			foreach (var allResourcesFiles in resourcesFiles) {
				// var guid = AssetDatabase.AssetPathToGUID (allResourcesFiles);
				// if (deleteFileList.Contains(guid)) {
				// 	continue;
				// }
				foreach (var path in AssetDatabase.GetDependencies (allResourcesFiles)) {
					if (path == allResourcesFiles) {
						continue;
					}
