...
public class TextureScaler
{
    ...
    public static Texture2D scaled(Texture2D src, int width, int height, int textureFormat, int mode = 1)
    {
		try
		{
			Rect texR = new Rect(0, 0, width, height);
			_gpu_scale(src, width, height, (FilterMode)mode);

			//Get rendered data back to a new texture
			Texture2D result = new Texture2D(width, height, (TextureFormat)textureFormat, true);
    ...
    /// <param name="mode">Filtering mode</param>
    public static void scale(Texture2D tex, int width, int height, int mode = 1)
    {
		try
		{
			Rect texR = new Rect(0, 0, width, height);
			_gpu_scale(tex, width, height, (FilterMode)mode);
