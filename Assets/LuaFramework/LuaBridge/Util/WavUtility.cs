...
public class WavUtility
{
	...
	private static float[] CustomConvert16BitByteArrayToAudioClipData (byte[] source, int headerOffset, int dataSize)
	{
    ...
    public static AudioClip CustomToAudioClip (string filePath, int sampleRate)
    {
        byte[] fileBytes = File.ReadAllBytes (filePath);
		float[] data = CustomConvert16BitByteArrayToAudioClipData (fileBytes, 0, fileBytes.Length);
		AudioClip audioClip = AudioClip.Create ("wav", data.Length, 1, sampleRate, false);
		audioClip.SetData (data, 0);
		return audioClip;
    }

    public static byte[] CustomFromAudioClip (AudioClip audioClip, float realLen, float totalLen)
    {
        int len = (int)(audioClip.samples * audioClip.channels * (realLen / totalLen));
