...
---@param t table
---@param num int
function CardTool.table_num_remain(t, num)
    local temp = {}
    for i, j in ipairs(t) do
        table.insert(temp, j % num)
    end
    return temp
end
...
---@return 随机返回table中的一个值
---@param t table
function CardTool.RandomOne(t)
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))  --随机种子  os.time()返回的数值字串倒过来再取高位7位。
    if type(t) ~= table then
        return nil
    end
    local randonindex = math.random(1, #t)
    return t[randonindex]
end
