--- 三公历史记录 module
--- Created by a.
--- DateTime: 2017/11/28 11:28
---

-- 初始化
local class = require("lib.middleclass")
local ModuleBase = require("core.mvvm.module_base")
local HistoryListModule = class("SanGong.HistoryListModule", ModuleBase)
...
function HistoryListModule:on_click(obj, arg)
    ...
    if obj == self.historyListView.buttonClose.gameObject then
        ModuleCache.ModuleManager.hide_module("sangong", "historylist")
    ...
    elseif obj.transform.parent.parent.parent.gameObject == self.historyListView.loopScrollView.gameObject then
        local loopBaseNode =  GetComponentWithPath(obj,"","LoopBaseNode") --ModuleCache.ComponentUtil.GetComponent(obj,"LoopBaseNode")
        local data = loopBaseNode.data
        ModuleCache.ModuleManager.show_module("sangong", "roomdetail", data)
    end
end
