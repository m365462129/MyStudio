...
function ActivityModule:on_click(obj, arg)
    ...
    if obj == self.view.buttonBack.gameObject then
        ...
        -- 邀请好友按钮
    elseif obj == self.view.buttonInviteFriend.gameObject then
        ModuleCache.ModuleManager.show_module("henanmj", "share");
    end
end
...
function ActivityModule:on_model_event_bind()
    ...
    local onGetImageShareConfig = function(eventHead, eventData)
        ...
        -- 更新微信视图
        self.view:updateWechatView(eventData.custosmerService);
    end
    -- 监听获取图片分享配置协议
    ...
    local onGetActivityByViewId = function(eventHead, eventData)
        ...
        -- 初始化
        self.view:updateActivityRightView(eventData);
    end
    -- 监听获取活动右侧列表事件
