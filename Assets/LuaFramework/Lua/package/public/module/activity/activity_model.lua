    ...
    local onResponse = function(wwwOperation)
        ...
        if retData.ret and retData.ret == 0 then
            ...
            for key, activity in ipairs(activityADList) do
                ...
                self:startDownLoadHeadIcon(activity.imgUrl, onStartComplete);
            end
        else
...
function ActivityModel:isEveryDayMustAppearUsingGET(dataShow)
    ...
    if UnityEngine.PlayerPrefs.GetString(str) ~= today then
        UnityEngine.PlayerPrefs.SetString(str, today)
    else
        return
    end
...
-- 下载头像
function ActivityModel:startDownLoadHeadIcon(url, callback)
