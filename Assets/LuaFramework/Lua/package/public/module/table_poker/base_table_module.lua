...
local TableManagerPoker = require('package.henanmj.table_manager_poker')
local Time = Time
...
function BaseTableModule:get_share_data()
	...
	if(self.modelData.roleData.HallID and self.modelData.roleData.HallID > 0) then
		...
		shareData.ruleName = name .. ' ' ..  ruleName
...
function BaseTableModule:getSeatInfoByPlayerId(playerId, seatInfoList)	
	...
	for i=1,#seatInfoList do
		if(seatInfoList[i].playerId == tmpPlyaerId or seatInfoList[i].playerId == tostring(tmpPlyaerId)) then
...
function BaseTableModule:is_open_shake()
    return (UnityEngine.PlayerPrefs.GetInt("openShake", 1) == 1)
end
...
function BaseTableModule:on_click_setting_btn(obj, arg)
	...
	ModuleCache.ModuleManager.show_module("henanmj", "roomsetting", data)		--根据房间是否开始的状态传值Moduel
end
...
function BaseTableModule:on_click_chat_btn(obj, arg)
	ModuleCache.ModuleManager.show_module("henanmj", "tablechat",{is_New_Sever=true, allChatMsgs=allChatMsgs, curTableData = self.chatCurTableData, config=self:getChatShowShotText()})
end
...
function BaseTableModule:refresh_table_bg()
	local tableBg = UnityEngine.PlayerPrefs.GetInt("RoomSetting_TableBackground_Name_" .. self.packageName, 1)
...
function BaseTableModule:getCurLocationSetting()
	...
	if config.isOpenLocationSetting then
		...
		locationSetting = UnityEngine.PlayerPrefs.GetInt(key,defSetting)
	end
