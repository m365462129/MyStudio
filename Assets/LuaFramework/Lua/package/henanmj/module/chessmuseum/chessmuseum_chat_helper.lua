---
--- Created by formatingxie.
--- DateTime: 2018/1/25 18:45
---
...
function ChatHelper:getInitChatData(mParlorId)
    chatMgr.getGroupTalk(function(messages)
        ...
        if ModuleCache.ModuleManager.module_is_active("henanmj", "chessmuseum") and self.module.view then
            self.module.view:initLoopScrollViewList_chatMsg(self.chatDataList)
        end
    end)
...
function ChatHelper:Init_wordMsg()
    ...
    self.onRecvOfflineMsg = function()
        if chatMgr.isLogin and self.module.selectDetailData.id  and self.imChatRoom then
...
function ChatHelper:on_click_voice_bubble(fildid)
    ...
    self.module:dispatch_package_event("Event_TableVoice_VoiceComing", data)
end
