...
function TableLocationModule:on_show(data)
    ...
    if data.gameType == "majiang" then
        ...
        -- 获取玩家信息列表(麻将专用)
        playerLocationList = self:getPlayerInfoListByMJ(data.seatHolderArray, data.tableCount);
    ...
    elseif data.gameType == "paohuzi" then
        playerLocationList = self:getPlayerLocationList(data.seatHolderArray);
    ...
    elseif data.gameType == "runfast" or data.gameType == "doudizhu" or data.gameType == "guandan" then
        ...
        -- 获取玩家信息列表(跑得快专用)
        playerLocationList = self:getPlayerInfoListByRunfast(data.seatHolderArray, data.tableCount);
    end
    ...
    if data.isShowLocation then
        ...
        if playerInfoCount > 2 then
            -- 是否有新的作弊玩家
            local isNewPlayerCheat = self:isHaveNewCheatPlayer(playerLocationList);
    ...
    self.view:init(playerLocationList);

    -- if data.buttonLocation then
    --        -- 是否开启定位按钮
    --        local isOpenLocation = self:isOpenLocation(playerLocationList);
    -- data.buttonLocation.gameObject:SetActive(isOpenLocation);
    -- end
end
...
function TableLocationModule:showJinPlayerErr(data)
    local str = ''

    

    local playerLocation2 = playerLocationList[2];

    if not playerLocation2 then
        return isPlayerCheat;
    end

    for key, locationObject in ipairs(playerLocation2.locationList) do
        if locationObject.isDistanceNear then
            str = locationObject.name1 .. '和' .. locationObject.name2 .. ' 位置小于100米'
        end
    end
    return str
end
...
function TableLocationModule:getPlayerInfoListByMJ(seatHolderArray, tableCount)
    ...
    -- print_table(seatInfoList);

    -- print("222222==", #seatInfoList, "====", tableCount);
    -- print_table(seatInfoList);
...
function TableLocationModule:getPlayerInfoListByRunfast(seatInfoList, tableCount)
    ...
    --    end
    --    -- 按玩家座位升序排序
    --    table.sort(seatInfoList, onSeatSort);
    --print(33333);
    --print_table(seatInfoList);
...
function TableLocationModule:isPlayerIpSame(playerInfo1, playerInfo2)
    ...
    if playerInfo1.ip == "0" and playerInfo2.ip == "0" then
        isSameIp = false;
