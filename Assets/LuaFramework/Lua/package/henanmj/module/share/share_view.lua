...
function ShareView:initialize(...)
    -- 初始View
    View.initialize(self, "henanmj/module/share/henanmj_windowshare.prefab", "HeNanMJ_WindowShare", 1)
    ...
    self.playerInfoDialog = GetComponentWithPath(self.root, "Center/BindInviteDialog", ComponentTypeName.Transform).gameObject;
    self.headImg = GetComponentWithPath( self.playerInfoDialog, "Center/Avatar/Avatar/Image",ComponentTypeName.Image)
    self.nickTex = GetComponentWithPath( self.playerInfoDialog, "Center/nickTex",ComponentTypeName.Text)
    self.idTex = GetComponentWithPath( self.playerInfoDialog, "Center/idTex",ComponentTypeName.Text)
    self.buttonConfirm = GetComponentWithPath( self.playerInfoDialog, "Center/ButtonConfirm",ComponentTypeName.Button)
    ...
    print(self.SpriteHolderGoods.name)
...
function ShareView:on_show_dialog(data,isShow)
    self.playerInfoDialog:SetActive(isShow)

    if data == nil then
        return
    end

    self.nickTex.text = data.nickname
    self.idTex.text = "ID:"..data.userId
...
function ShareView:updateShareImage(data, spriteShareImage, spriteShareBg)
    ...
    if self.shareImagePrefab == nil then
        -- 实例化分享图片预置
        self.shareImagePrefab = ViewUtil.InitGameObject("henanmj/module/shareimage/shareimage.prefab", "ShareImage", self.shareCanvasGB);
        -- 二维码图片
