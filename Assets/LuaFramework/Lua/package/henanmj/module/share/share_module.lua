...
function ShareModule:initJiangLi()
...
function ShareModule:on_click(obj, arg)
    ...
    if obj == self.shareView.buttonClose.gameObject then
        ModuleCache.ModuleManager.hide_module("henanmj", "share")
    ...
    elseif obj.name == "ButtonCancel" then
        self.view:on_show_dialog(nil, false)
    ...
    self:http_get(requestData, function(wwwData)
        ...
        if (retData.success) then
            if retData.data then
                self.view:on_show_dialog(retData.data, true)
            end
        else
            if retData.errMsg then
...
function ShareModule:on_show(data)
    if data == "hall" then
        ...
        if (data) then
            ...
            self.shareData.parlorId = data.parlorNum .. ""
        else
...
-- 下载头像
function ShareModule:startDownLoadHeadIcon(url, callback)
