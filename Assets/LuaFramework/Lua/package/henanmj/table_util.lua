...
function TableUtil.set_tag(mj, tagSpriteH, tagVal)
    ...
    if(tag) then
        local tagImage = GetComponentWithPath(mj, "Tag", ComponentTypeName.Image)
        tag.gameObject:SetActive(true)
        tagImage.sprite = tagSpriteH:FindSpriteByName(tagVal .. "")
    end
end
...
-- 下载头像
function TableUtil.start_download_head_icon(playerInfo, callbackHead)
...
function TableUtil.get_rule_name(rule,  isMuseumRoom) --isMuseumRoom 亲友圈
    print("-------------------isMuseumRoom:",isMuseumRoom)
