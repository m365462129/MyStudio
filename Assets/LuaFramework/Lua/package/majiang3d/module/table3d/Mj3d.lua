...
function Mj3d:Create(pai,parent)
    ...
    mj3d.skinObjs = {}
    for i=1,#skinStyle do
        local objName = "Style_"..tostring(i)
        local objs = {}
        table.insert(objs,GetComponentWithPath(mj3d.meshObj,objName,ComponentTypeName.Transform).gameObject)
        table.insert(objs,GetComponentWithPath(mj3d.hMeshObj,objName,ComponentTypeName.Transform).gameObject)
        table.insert(objs,GetComponentWithPath(mj3d.outlineMeshObj,objName,ComponentTypeName.Transform).gameObject)
        table.insert(objs,GetComponentWithPath(mj3d.grayMeshObj,objName,ComponentTypeName.Transform).gameObject)
        table.insert(mj3d.skinObjs,objs)
    end
...
function Mj3d:setSkinStyle(type)
    for i=1,#self.skinObjs do
        local isActive = (i == type)
        local Objs = self.skinObjs[i]
        for j=1,#Objs do
            local obj = Objs[j]
