...
local TableShiSanZhangHelper = {}
local ModuleCache = ModuleCache
local GetComponentWithPath = ModuleCache.ComponentUtil.GetComponentWithPath
local Sequence = DG.Tweening.DOTween.Sequence

function TableShiSanZhangHelper:setSeatPlayerInfo(seatHolder, playerInfo)

end

function TableShiSanZhangHelper:initDealTable(root)
    self.inhandPokers = {}
    self.matching = {};
    self.animMatching = {};
    TableShiSanZhangHelper:initInHandPokers(root);
    for i = 1, 3 do
        TableShiSanZhangHelper:initMatching(root,i);
    end
    TableShiSanZhangHelper:initAnimMatching(root);
end

function TableShiSanZhangHelper:initInHandPokers(root)
    for i = 1,9 do
...
function TableShiSanZhangHelper:initMatching(root,index)
    self.matching[index] = {};
    local path;
    if(index == 1) then
...
function TableShiSanZhangHelper:initAnimMatching(root)
    for i = 1,3 do
...
function TableShiSanZhangHelper:initSeatHolder(seatHolder, seatIndex, goSeat, handPokerRoot)
    local root = goSeat
...
function TableShiSanZhangHelper:initCardHolder(cardHolder, pokerAssetHolder)
...
function TableShiSanZhangHelper:PlayAnimHandToMatch(handIndex,matchIndex,detailIndex,onFinishInvoke)
...
function TableShiSanZhangHelper:PlayAnimMatchToMatch(srcIndex,desIndex,onFinish)
    local duration = 0.5;
...
function TableShiSanZhangHelper:refreshInHandCards(seatHolder, inHandPokerList, showFace, useAnim)
...
function TableShiSanZhangHelper:playCardTurnAnim(cardHolder, toFace, duration, delayTime, onFinish)
...
function TableShiSanZhangHelper:playCardFlyToPosAnim(cardHolder, targetPos, duration, delayTime, onFinish)
    local sequence = self.module:create_sequence()
...
function TableShiSanZhangHelper:playCardScaleAnim(cardHolder, targetScale, duration, delayTime, onFinish)
    local sequence = self.module:create_sequence()
...
--显示牛几
function TableShiSanZhangHelper:showNiuName(seatHolder, show, niuName)    
...
function TableShiSanZhangHelper:setInHandPokersDonePos(seatHolder)
...
function TableShiSanZhangHelper:setInHandPokersOriginalPos(seatHolder)
    seatHolder.handPokerCardsRoot.transform.position = seatHolder.inHandCardsOriginalPos
end


--播放随机选庄动画
function TableShiSanZhangHelper:playRandomBankerAnim(seatHolderList, targetSeat, onFinish)
...
--播放设置庄家动画
function TableShiSanZhangHelper:playSetTargetSeatAsBanker(seatHolder, onFinish)
...
--刷新座位相关信息
function TableShiSanZhangHelper:refreshSeatInfo(seatHolder, seatData)  
    if(seatData.playerId and (seatData.playerId ~= 0 and seatData.playerId ~= "0")) then        
        ...
        ModuleCache.ComponentUtil.SafeSetActive(seatHolder.imageCreator.gameObject, tonumber(self.modelData.curTableData.roomInfo.roomHostID) == tonumber(seatData.playerId) and self.modelData.roleData.RoomType == 2)
        ...
        --快速组局不显示踢人
        ModuleCache.ComponentUtil.SafeSetActive(seatHolder.buttonKick.gameObject, seatData.canBeKicked )
...
function TableShiSanZhangHelper:refreshSeatOfflineState(seatHolder, seatData)
    ...
    if self.modelData.roleData.RoomType == 2 and tonumber(self.modelData.curTableData.roomInfo.curRoundNum) == 0 then
        ...
        and tonumber(seatData.playerId) ~= tonumber(self.modelData.curTableData.roomInfo.mySeatInfo.playerId) then
            ...
            ModuleCache.ComponentUtil.SafeSetActive(seatHolder.buttonKick.gameObject, seatData.isOffline)
        end
...
--刷新座位的状态
function TableShiSanZhangHelper:refreshSeatState(seatHolder, seatData)
    if(seatData.playerId and (seatData.playerId ~= 0 and seatData.playerId ~= "0")) then        
...
function TableShiSanZhangHelper:showSeatWinScoreCurRound(seatHolder, show, score)
...
function TableShiSanZhangHelper:refreshClock(seatHolder, show, targetTime, curTime)
...
--显示或隐藏说话icon
function TableShiSanZhangHelper:showSpeakIcon(seatHolder, show)
    ModuleCache.ComponentUtil.SafeSetActive(seatHolder.goSpeakIcon, show) 
end

--显示下注按钮
function TableShiSanZhangHelper:setBetScore(seatHolder, seatData, show)    
...
--填写玩家信息
function TableShiSanZhangHelper:setPlayerInfo(seatHolder, playerData)
...
--下载头像
function TableShiSanZhangHelper:startDownLoadHeadIcon(targetImage, url, callback)    
...
--设置牌的数据
function TableShiSanZhangHelper:setCardInfo(cardHolder, poker)    
...
--显示或隐藏牌
function TableShiSanZhangHelper:showCard(cardHolder, show)
    ModuleCache.ComponentUtil.SafeSetActive(cardHolder.cardRoot, show)
end

--显示牌正面
function TableShiSanZhangHelper:showCardFace(cardHolder)
...
--显示牌背面
function TableShiSanZhangHelper:showCardBack(cardHolder)
...
--显示或隐藏手牌
function TableShiSanZhangHelper:showInHandCards(seatHolder, show)    
    ModuleCache.ComponentUtil.SafeSetActive(seatHolder.handPokerCardsRoot, show)
end

function TableShiSanZhangHelper:getImageNameFromPoker(poker)
...
function TableShiSanZhangHelper:getImageNameFromNiuName(niuName)
    print(niuName)
    return niuName
end


function TableShiSanZhangHelper:getSoundNameFromNiuName(niuName)
    local headStr = "cow_"
...
function TableShiSanZhangHelper:getNumberFormPoker(poker)
    if(poker.number == "A") then
        return 1
    elseif (poker.number == "J") then 
        return 10
...
function TableShiSanZhangHelper:checkHasNiuFormPokerArray(pokerArray, result)
    result = result or {}
...
function TableShiSanZhangHelper:getSeatInfoByRemoteSeatIndex(remoteSeatIndex, seatInfoList)	
...
--将服务器的做座位索引转换为本地位置索引
function TableShiSanZhangHelper:getLocalIndexFromRemoteSeatIndex(seatIndex, mySeatIndex, seatCount)
...
--判断是否所有入座的玩家都已准备
function TableShiSanZhangHelper:checkIsAllReady(seatInfoList)
...
--通过玩家id获取座位信息
function TableShiSanZhangHelper:getSeatInfoByPlayerId(playerId, seatInfoList)	
...
function TableShiSanZhangHelper:getSeatedSeatCount(seatInfoList)
	local count = 0    
	for i=1,#seatInfoList do
...
function TableShiSanZhangHelper:get_userinfo(playerId, callback)             
    ...
    --self.module = require("package/shisanzhang/module/tableshisanzhang/tableshisanzhang_module")
...
--播放牛名
function TableShiSanZhangHelper:playNiuNameSound(niuName, isFemale)
    ...
    ModuleCache.SoundManager.play_sound("bullfight", "bullfight/sound/table/" .. soundName .. ".bytes", soundName)
end



function TableShiSanZhangHelper:showNiuNiuEffect(seatHolder, show, duration, stayTime, delayTime, onComplete)
...
function TableShiSanZhangHelper:showRoundScoreEffect(seatHolder, localSeatIndex, show, score, duration, delayTime, stayTime, onComplete)
...
function TableShiSanZhangHelper:showRandomBankerEffect(seatHolder, show)
    ModuleCache.ComponentUtil.SafeSetActive(seatHolder.animSelectBanker.gameObject, show)    
end


function TableShiSanZhangHelper:formatRuleDesc(rule)
    local desc = ""
