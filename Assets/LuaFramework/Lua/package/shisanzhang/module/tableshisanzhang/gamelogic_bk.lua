...
Cal.pow2={Cal.pow(2,1),Cal.pow(2,2),Cal.pow(2,3),Cal.pow(2,4),Cal.pow(2,5),Cal.pow(2,6),Cal.pow(2,7),
			Cal.pow(2,8),Cal.pow(2,9),Cal.pow(2,10),Cal.pow(2,11),Cal.pow(2,12),Cal.pow(2,13)}
...
function Cal.GenerateHBT(poker,gpai)
	...
	if Cal.IsShiSanZhang(poker) then
		pai[1]={poker[13],poker[1],poker[2]}
		pai[2]={poker[3],poker[4],poker[5],poker[6],poker[7]}
		pai[3]={poker[8],poker[9],poker[10],poker[11],poker[12]}
		pai.XiPaiType=4
		for i=#hbts,1,-1 do
			if hbts[i].XiPaiType>0 then
				table.remove(hbts,i)
	...
	elseif Cal.IsLiuDuiBan(poker) then
        ...
        if flag then 
			...
			for i=1,13 do
				...
				elseif i==13 then
					if poker[12].Number~=poker[13].Number then
						danzhang=13
					end
				elseif poker[i].Number~=poker[i-1].Number and poker[i].Number~=poker[i+1].Number then
					danzhang=i
				end
				if danzhang then
					break
    ...
    for i = 1, #HBTs - 1 do
        ...
        for j = i + 1, #HBTs do
			...
			if HBTs[j].XiPaiType>0 or HBTs[maxpos].XiPaiType>0 then
				flag=HBTs[j].XiPaiType>HBTs[maxpos].XiPaiType
			else 
