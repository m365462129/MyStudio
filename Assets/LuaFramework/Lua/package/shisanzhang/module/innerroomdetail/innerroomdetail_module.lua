...
local InnerRoomDetailModule = class("BullFight.InnerRoomDetailModule", ModuleBase)
...
function InnerRoomDetailModule:initialize(...)
    -- 开始初始化                view        loginModel           模块数据
    ModuleBase.initialize(self, "innerRoomDetail_view", "innerRoomDetail_model", ...)

    --  self.roomDetailView:initLoopScrollViewList( { roundList = { } })
end


function InnerRoomDetailModule:on_show(roomInfo)
    self.creatorId = roomInfo.creatorId
    self.innerRoomDetailView:initRoomInfo(roomInfo)
    self:getRoomList(roomInfo)
end


function InnerRoomDetailModule:on_click(obj, arg)
    ModuleCache.SoundManager.play_sound("henanmj", "henanmj/sound/button.bytes", "button")
    print(obj.gameObject.name);
    if obj.name == "ButtonClose" then
        ModuleCache.ModuleManager.hide_module("shisanzhang", "innerroomdetail")
        return
        --    elseif obj == self.roomDetailView.buttonBack.gameObject then
        --        ModuleCache.ModuleManager.hide_module("bullfight", "roomdetail")
        --        print(2222);
        --        return
...
function InnerRoomDetailModule:getRoomList(roomInfo)
    local requestData = {
    ...
    self:http_get(requestData, function(wwwData)
        ...
        if (retData.ret and retData.ret == 0) then
            ...
            self.innerRoomDetailView:initLoopScrollViewList(roundList,self.creatorId)
            if(#roundList == 0) then
                self.innerRoomDetailView.noInfo:SetActive(true)
            else
                self.innerRoomDetailView.noInfo:SetActive(false)
            end
        else
...
function InnerRoomDetailModule:sortPlayer(data)
    local newPlayerList = {}
--    local index = 2
...
function InnerRoomDetailModule:parseRoundList(data)
...
function InnerRoomDetailModule:DecodeRecord(record)
    local curString = "";
    ...
    for i = 1, recordLength do
        ...
        if(curChar == "|") then
            ...
            elseif(curCount == 16) then
                recordInfo.soloKillScore = tonumber(curString);
            elseif(curCount == 17) then
                recordInfo.allKillScore = tonumber(curString);
            elseif(curCount == 18) then
                recordInfo.spadeAScore = tonumber(curString);
            end
            curString = "";
        else
...
function InnerRoomDetailModule:DecodeMatchRecord(record)
    local match = {};
    local curStrPoker = "";
...
function InnerRoomDetailModule:DecodeXipaiRecord(record)
    local match = {};
...
function InnerRoomDetailModule:DecordXipaiAndScore(strXipai)
    local curStrXipaiInfo = "";
    local xipai = "";
    local score = 0;
...
local xipaiTypeTab = {"三花茶", "三顺子",  "六对半", "十三张", "半大", "半小", "全大", "全小", "五对三条",
                        "全黑", "全红", "全黑一点红", "全红一点黑", "四套三条", "清一色龙"}

function InnerRoomDetailModule:DecodeXipaiType(xipaiType)
    ...
    --end
    --return strXipai;
    return xipaiTypeTab[tonumber(xipaiType)] or ""
end

function InnerRoomDetailModule:DecordStrPoker(strPoker)
...
function InnerRoomDetailModule:getPlayerInfoBySeatId(seatId, playerList)
...
function InnerRoomDetailModule:getPlayerInfoByName(name, playerList)
    for i, v in ipairs(playerList) do
...
function InnerRoomDetailModule:getPlayerNameBySeatId(seatId,playerList)
    for i, v in ipairs(playerList) do
        if (seatId == v.seatId) then
            return v.playerName
        end
...
function InnerRoomDetailModule:getUserIdBySeatId(seatId,playerList)
    for i, v in ipairs(playerList) do
        if (seatId == v.seatId) then
            return v.userId
        end
...
function InnerRoomDetailModule:getPlayerIndex( userId,playerList )
    for i, v in ipairs(playerList) do
