...
function TableCPBaseModule:initialize(...)
...
function TableCPBaseModule:on_module_inited(...)
...
function TableCPBaseModule:getTableKeyCount(t)
	local count = 0
...
function TableCPBaseModule:on_module_event_bind()
...
function TableCPBaseModule:on_send_gift_chat_msg(seatId, content)
	if(string.sub(content, 1, 1) == "{")then
		local gift = ModuleCache.Json.decode(content)
		local senderLocalSeatIndex = TableUtil.get_local_seat(seatId, self.view.mySeat, TableManager.curTableData.totalSeat)
		local receiverLocalSeatIndex = TableUtil.get_local_seat(gift.receiver, self.view.mySeat, TableManager.curTableData.totalSeat)
...
-- 离开房间
function TableCPBaseModule:exit_room(tip)
	...
	if UnityEngine.PlayerPrefs.GetInt("ChangeTable", 0) ==0 then
		...
		nil,
			ModuleCache.UnityEngine.PlayerPrefs.GetString("LastJoinWanfaName",""),
		nil,
			ModuleCache.UnityEngine.PlayerPrefs.GetInt("LastJoinGoldFieldID",1)
		)
		--print("-----------------join_room---:",UnityEngine.PlayerPrefs.GetString("LastWanfaName","QQSXQP_SXMJ_SX"),
		--	UnityEngine.PlayerPrefs.GetInt("LastGoldFieldID",1)
		--)
...
function TableCPBaseModule:press_up_voice(obj, arg)
	local data = {
		obj = obj,
		arg = arg,
	}
...
function TableCPBaseModule:on_drag_voice(obj, arg)
...
function TableCPBaseModule:press_voice(obj, arg)
	if(obj == self.view.buttonMic.gameObject) then
		local data = {
			obj = obj,
			arg = arg,
...
function TableCPBaseModule:on_show()
...
--function TableCPBaseModule:on_hide()
--	UpdateBeat:Remove(self.UpdateBeat, self)
--end

function TableCPBaseModule:on_destroy()
	--UpdateBeat:Remove(self.UpdateBeat, self)
...
function TableCPBaseModule:reconnect()
	
end

function TableCPBaseModule:on_update_per_second()
...
function TableCPBaseModule:PauseMusic()
	SoundManager.audioMusic.mute = true
end

function TableCPBaseModule:UnPauseMusic()
	SoundManager.audioMusic.mute = false
end

--- 刷新单个玩家状态
function TableCPBaseModule:refresh_seat_info(data)
...
function TableCPBaseModule:update_user_state()
	self.curPlayer = 0
...
function TableCPBaseModule:refresh_user_state(data)
...
function TableCPBaseModule:get_share_data_copy_to_clipboard()
	if(not self.gameState and not self.curTableData.isPlayBack and self.curTableData.RoomType ~= 4) then
...
--- 刷新游戏状态
function TableCPBaseModule:refresh_game_state(gameState)

end

function TableCPBaseModule:get_share_data()
    ...
    if shareData.rule.PayType == 1 then
        shareData.ruleName = shareData.ruleName .. "AA支付"
    elseif shareData.rule.PayType == 0 then
        shareData.ruleName = shareData.ruleName .. "房主支付"
    elseif shareData.rule.PayType == 2 then
        shareData.ruleName = shareData.ruleName .. "大赢家支付"
    end
...
--- 邀请好友
function TableCPBaseModule:inviteWeChatFriend()
