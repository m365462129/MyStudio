    ...
    elseif obj == self.historyListView.buttonLookMatch.gameObject then
        ModuleCache.ModuleManager.show_module("henanmj", "playvideo", function(data)
            if(data.ret and data.ret == 0)then
                ...
                self:playVideo(recordID,data.data.players)
            else
...
function HistoryListModule:getRoomList(roomInfo)
    ...
    self:http_get(requestData, function(wwwOperation)
        ...
        if retData.ret and retData.ret == 0 then
            ...
            ModuleCache.ModuleManager.show_module(BranchPackageName, "roomdetail", retData.data)
        else
...
function HistoryListModule:playVideo(recordId,players)
    self:get_videoData(recordId, function(data,gamestates)
        self:on_videoData(data,gamestates,players)
    end)
end

function HistoryListModule:on_videoData(data,gamestates,players)
    local index = 1
    for i = 1, #players do
...
function HistoryListModule:get_videoData(recordId, callback)
    TableManager:get_play_back_info(recordId,function(gamestates, data)
        print_table(data)
        if(callback)then
            callback(data,gamestates)
        end
    end)
end
