...
function RoomDetailView:initialize(...)
    View.initialize(self, "daigoutui/module/roomdetail/daigoutui_windowroomdetail.prefab", "DaiGouTui_WindowRoomDetail", 1)
...
function RoomDetailView:updatePlayerView(playerList)
    ...
    for i = 1, 5 do
        ...
        -- 房主图标
        local spriteRoomOwner = GetComponentWithSimple(labelPlayerName.gameObject, "SpriteRoomOwner", ComponentTypeName.Image);
    ...
    -- 获取斗地主规则描述
    self.labelRuleName.text = self:GetDaiGouTuiRuleText(self.roomList.playRule);
end


-- 获取斗地主规则描述
function RoomDetailView:GetDaiGouTuiRuleText(RuleJsonString)
    local desc = ""
    local locJsonString = ""
