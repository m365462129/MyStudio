...
local offsetY = 80
local first_row_max_col_count = 20
...
function HandPokers:initialize(module)
    ...
    self.sort_pattern_type = UnityEngine.PlayerPrefs.GetInt('daigoutui_sort_pattern_type', 1)
end

function HandPokers:set_sort_pattern_type(type)
    self.sort_pattern_type = type or 1
    UnityEngine.PlayerPrefs.SetInt('daigoutui_sort_pattern_type', self.sort_pattern_type)
end
...
function HandPokers:repositionPokers(colList, withoutAnim, onFinish)
    ...
    for i=1,colCount do
        for j=#colList[i],1,-1 do
            ...
            self:showPokerServantCardTag(pokerHolder, pokerHolder.poker.code == self.servantCard)
        end
    end
    if(colCount == 0)then
        if(onFinish)then
...
function HandPokers:genPokerHolder(poker, show)
    ...
    if(not pokerHolder)then
        ...
        pokerHolder.imageServantCardTag = GetComponentWithPath(pokerHolder.root, "servantTag", ComponentTypeName.Image)
        pokerHolder.imageServantCardIconTag = GetComponentWithPath(pokerHolder.root, "gouTuiTag", ComponentTypeName.Image)
...
function HandPokers:showPokerServantCardTag(pokerHolder, show)
    ModuleCache.ComponentManager.SafeSetActive(pokerHolder.imageServantCardTag.gameObject, show or false)
    ModuleCache.ComponentManager.SafeSetActive(pokerHolder.imageServantCardIconTag.gameObject, show or false)
end

function HandPokers:showPokerLordTag(pokerHolder, show)
    ModuleCache.ComponentManager.SafeSetActive(pokerHolder.goLordTag, show or false)
end

function HandPokers:showPokerMingPaiTag(pokerHolder, show)
    ModuleCache.ComponentManager.SafeSetActive(pokerHolder.goMingPaiTag, show or false)
end
