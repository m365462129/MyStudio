...
function TableModule:on_click(obj, arg)	
	...
	elseif (obj.name == "ButtonBackToSettle") then
		if self.view.ConfigData.isHuangShiHH then --现在只有黄石晃晃有这个需求
			ModuleCache.ModuleManager.show_module("majiang", "onegameresultpuning",self.presettlementState)
		end
	...
	self:subscibe_time_event(waitTime, false, 0):OnComplete(function(t)
		if(self:can_show_result()) then
			...
			if self.view.ConfigData.pnShowResult then
				ModuleCache.ModuleManager.show_module("majiang", "onegameresultpuning", newGameState)
			else
				ModuleCache.ModuleManager.show_module("majiang", "onegameresult", newGameState)
			end
		end
	end)
end
...
function TableModule:update_early_start()
	...
	if curPlayerNum < curMaxPlayerNum  and  curPlayerNum > 1 then
