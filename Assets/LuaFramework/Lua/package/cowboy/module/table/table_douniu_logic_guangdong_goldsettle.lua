...
--- @class TableDouNiuLogic_GuangDong_GoldSettle:TableDouNiuLogic_GuangDong
...
local TableDouNiuLogic_GuangDong_GoldSettle = class('TableDouNiuLogic_GuangDong_GoldSettle', TableDouNiuLogic)
local CSmartTimer = ModuleCache.SmartTimer.instance

function TableDouNiuLogic_GuangDong_GoldSettle :initialize(...)
...
--同步消息
function TableDouNiuLogic_GuangDong_GoldSettle:on_table_synchronize_notify(data)
...
function TableDouNiuLogic_GuangDong_GoldSettle:on_table_start_notify(data)
    TableDouNiuLogic.on_table_start_notify(self, data)
    local roomInfo = self.modelData.curTableData.roomInfo
    self.tableView:showGoldCoinDiZhu(true, roomInfo.ruleTable.baseScore, roomInfo.curRoundNum, roomInfo.totalRoundCount)
end

function TableDouNiuLogic_GuangDong_GoldSettle:on_table_ago_settle_accounts_notify()

end

function TableDouNiuLogic_GuangDong_GoldSettle:showGameResult(data)
    data.is_gold_settle = true
    TableDouNiuLogic.showGameResult(self, data)
end

function TableDouNiuLogic_GuangDong_GoldSettle:playRoundResultScore(seatInfo, roundScore)
    local roomInfo = self.modelData.curTableData.roomInfo
    local gold = roundScore * roomInfo.ruleTable.baseScore
    TableDouNiuLogic.playRoundResultScore(self, seatInfo, gold)
end

return TableDouNiuLogic_GuangDong_GoldSettle