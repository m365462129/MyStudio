...
function RoomDetailView:initialize(...)
    -- 初始View
    View.initialize(self, "majiangshanxi/module/roomdetail/henanmj_windowroomdetail.prefab", "HeNanMJ_WindowRoomDetail", 1)
...
function RoomDetailView:initRoomInfo(data)
    ...
    local wanfaName = ""
    local ruleName = ""
    wanfaName, ruleName = TableUtil.get_rule_name(data.roomInfo.playRule);
    -- 玩法信息
...
function RoomDetailView:fillItem(item, index)
    ...
    if (data.breakEnd) then
        ...
        ComponentUtil.SafeSetActive(breakEndText.gameObject, true)
        ...
        ComponentUtil.SafeSetActive(breakEndText.gameObject, false)
        for i = 1, 4 do
