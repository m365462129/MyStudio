...
function HistoryListView:initialize(...)
    -- 初始View
    View.initialize(self, "majiangshanxi/module/historylist/henanmj_windowhistorylist.prefab", "HeNanMJ_WindowHistoryList", 0)
...
function HistoryListView:fillItem(item)
    ...
    for i = 1, 4 do
        ...
        if (i <= #data.players) then
            ...
            local score = data.players[i].playerScore
            if data.players[i].ratio and 1 ~= data.players[i].ratio then ---处理积分系数，现阶段只有黄石晃晃
                score = data.players[i].playerScore * data.players[i].ratio
            end
            if (score > 0) then
