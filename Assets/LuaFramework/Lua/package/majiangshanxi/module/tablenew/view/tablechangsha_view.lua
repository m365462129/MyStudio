...
---@class TableChangShaView:TableCommonView
local TableChangShaView = class('tableChangShaView', Base)
...
--- 初始化ui
function TableChangShaView:init_ui()
    Base.init_ui(self)
...
function TableChangShaView:init_config()
...
function TableChangShaView:get_action_tx(action)
...
---显示听牌标志
function TableChangShaView:show_ting_pai_tag(localSeat, show)

end

---可胡
function TableChangShaView:show_ke_hu(show)
...
---可杠 长沙变成可补
function TableChangShaView:show_ke_gang(show)
...
---长沙麻将可杠
function TableChangShaView:show_ke_gang2(show)
...
--- 隐藏操作以及弹出的操作列表
function TableChangShaView:hide_wait_action_select_card()
...
--- 开始刷新gameState
function TableChangShaView:game_state_begin(gameState)
...
--- 显示起手胡
function TableChangShaView:show_qi_shou_hu(gameState)
    self.waitActionCS:SetActive(false)
...
--- 显示开杠之后的操作（不是自己开杠的）
function TableChangShaView:show_kai_gang(gameState)
    self.csSelectKaiGang:SetActive(false)
    if(gameState.GangPai and #gameState.GangPai > 0) then
...
--- 显示海底捞
function TableChangShaView:show_hai_di_lao(show)
    self.selectHaiDi:SetActive(show)
end

--- 显示开杠详情
function TableChangShaView:show_kai_gang_detail(seatId)
    ...
    for i = 1, #self.gameState.GangPai do
        ...
        self:set_kai_gang_data(actionObj, selectCardPanel, self.gameState.GangPai[i])
    end
...
--- 设置开杠数据
function TableChangShaView:set_kai_gang_data(actionObj, selectCardPanel,  pai)
...
--- 可吃 针对开杠
function TableChangShaView:ke_chi_kai_gang(pai)
...
--- 开杠的牌有可操作的
function TableChangShaView:have_action_kai_gang(pai,list)
...
--- 显示开杠的牌的多种操作（多吃等）
function TableChangShaView:show_select_card_kai_gang(actionObj)
...
--- 牌是否亮倒
function TableChangShaView:pai_is_open(playerState, handData)
    return self.curTableData.isPlayBack or (handData and handData.Pai ~= 0 and (#playerState.HuPai ~= 0 or handData.State == 2))
end

--- 牌是否扣倒
function TableChangShaView:pai_is_covered(handData)
    return false
end

return  TableChangShaView