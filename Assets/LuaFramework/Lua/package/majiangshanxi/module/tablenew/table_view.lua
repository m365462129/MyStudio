...
local ViewBase = require('package.majiangshanxi.module.tablebase.tablebase_view')
--- @class TableMJView
...
function TableView:initialize(...)
    self:on_initialize("majiangshanxi/module/table/henanmj_table.prefab", "HeNanMJ_Table", 0)
    self:init_config()
    self:init_ui()
    self:init_play_mode()
...
function TableView:init_config()
    ...
    self.wanfaName,self.ruleName,self.totalSeat = TableUtil.get_rule_name(self.gamerule, self.curTableData.HallID > 0)
end
...
function TableView:init_play_mode()
    ...
    if(self.curTableData.RoomType == 2) then
        ...
        self.roomType = require('package.majiangshanxi.module.tablenew.roomtype.table_type_fast'):new(self)
    elseif(self.curTableData.RoomType == 3) then
        self.roomType = require('package.majiangshanxi.module.tablenew.roomtype.table_type_match'):new(self)
    elseif(self.curTableData.RoomType == 4) then
        self.roomType = require('package.majiangshanxi.module.tablenew.roomtype.table_type_gold'):new(self)
    elseif(self.curTableData.ruleJsonInfo.settleType == 1) then
        self.roomType = require('package.majiangshanxi.module.tablenew.roomtype.table_type_gold_result'):new(self)
    else
        self.roomType = require('package.majiangshanxi.module.tablenew.roomtype.table_type_common'):new(self)
    end

    ---模式 正常 回放
    if(not self.curTableData.isPlayBack) then
        ---@type TableCustom
        self.playType = require('package.majiangshanxi.module.tablenew.playtype.table_custom'):new(self)
    else
        self.playType = require('package.majiangshanxi.module.tablenew.playtype.table_playback'):new(self)
    end
end
...
function TableView:setRightMenuState(state)
    ...
    local closeEixtBtn = (4 == self.curTableData.RoomType)
...
function TableView:action_active()
    return self.waitAction.activeSelf
end
...
--- 播放打骰子
function TableView:play_dice(endCall)
...
function TableView:hide_table_pop()
    if(ModuleCache.ModuleManager.module_is_active("majiangshanxi", "tablepop")) then
        ModuleCache.ModuleManager.hide_module("majiangshanxi", "tablepop")
    end
    if(ModuleCache.ModuleManager.module_is_active("majiangshanxi", "tablestrategy")) then
        ModuleCache.ModuleManager.hide_module("majiangshanxi", "tablestrategy")
    end
    if(ModuleCache.ModuleManager.module_is_active("majiangshanxi", "tablepop2")) then
        ModuleCache.ModuleManager.hide_module("majiangshanxi", "tablepop2")
    end
end
...
function TableView:show_table_pop(userState, i)
    if(self.ConfigData.isStrategy) then
        if(not ModuleCache.ModuleManager.module_is_active("majiangshanxi", "tablestrategy")) then
            ModuleCache.ModuleManager.show_module("majiangshanxi", "tablestrategy", self.mySeat == userState.ZhuangJia)
        end
    end
    ...
    if(self.ConfigData.isTablePop2) then
        if(not ModuleCache.ModuleManager.module_is_active("majiangshanxi", "tablepop2")) then
            self.ruleJsonInfo.ZhuangJia = (self.mySeat == userState.ZhuangJia)
            ModuleCache.ModuleManager.show_module("majiangshanxi", "tablepop2", self.ruleJsonInfo)
        end
    end
end
...
function TableView:play_voice(path)
    ...
    if(not string.find(path, "common")) then
        if(self.ConfigData.soundPath) then
            path = self.ConfigData.soundPath .. "/" .. path
        end
    end
    ModuleCache.SoundManager.play_sound("majiangshanxi", "majiangshanxi/sound/".. path .. ".bytes", array[#array])
end
...
function TableView:game_state_lai_gen(show)
    ...
    self.laiGenMJ1:SetActive(false);
    if(not show) then
        return
    end
    ---@type Mj2D
...
function TableView:show_ding_que(allQue)
    ...
    if(not allQue) then
        ModuleCache.ModuleManager.show_module("majiangshanxi", "tableadd")
    else
        ModuleCache.ModuleManager.hide_module("majiangshanxi", "tableadd")
        --显示头像上的条，万，筒
...
function TableView:get_chat_short_text(index)
    if(self.ConfigData.chatShotTextList) then
