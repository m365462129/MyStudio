...
--- 显示买马 结算界面
function TableCommonController:show_mai_ma(gameState)
    local waitTime = 2
    ...
    self.module:subscibe_time_event(waitTime, false, 0):OnComplete(function(t)
        if(self.module:can_show_result()) then
            ...
            if self.view.ConfigData.pnShowResult then
                ModuleCache.ModuleManager.show_module("majiangshanxi", "onegameresultpuning", gameState)
            else
                ModuleCache.ModuleManager.show_module("majiangshanxi", "onegameresult", gameState)
            end
        end
    end)
end
...
--- 过胡提示
function TableCommonController:guo_hu_action(callback)
