...
function OneGameResultView:initialize(...)
    ...
    View.initialize(self, "majiangshanxi/module/onegameresult/henanmj_onegameresult_puning.prefab", "HeNanMJ_OneGameResult_PuNing", 0)
    ...
    self.textEndTime = GetComponentWithPath(self.root, "TopRight/Child/EndTime", ComponentTypeName.Text)

    self.button_countDownTex = GetComponentWithPath(self.btnRestart, "Count down/Text", ComponentTypeName.Text)
    GetComponentWithPath(self.btnRestart, "Image", ComponentTypeName.Transform).gameObject:SetActive(curTableData.RoomType ~= 3 and curTableData.RoomType ~= 4)
    GetComponentWithPath(self.btnRestart, "Count down", ComponentTypeName.Transform).gameObject:SetActive(curTableData.RoomType == 3 or curTableData.RoomType == 4)
    ...
    ComponentUtil.SafeSetActive(self.btnReturn, 4 == curTableData.RoomType) ---金币场显示返回按钮
    self.goldMode = (4 == curTableData.RoomType or curTableData.ruleJsonInfo.settleType == 1)
    ...
    if self.ConfigData.isHuangShiHH then  ---黄石晃晃打开小结算牌桌回放
    ComponentUtil.SafeSetActive(self.btnPreTabel, true)
    else
...
function OneGameResultView:refresh_view(newGameState)
    ...
    for i=1,4 do
        ...
        if(i<=curTableData.totalSeat) then
            ...
            ComponentUtil.SafeSetActive(playerTranData.goldTag, curTableData.RoomType == 4 or curTableData.ruleJsonInfo.settleType == 1)
    ...
    if 4 == curTableData.RoomType then
        self.textRoomID.text = TableUtil.get_rule_name(curTableData.Rule)
        self.textWanfa.text = "底注:"..gameState.BaseCoinScore
        self.textJuShu.text = ""
    end
    ...
    if(curTableData.RoomType == 3 and gameState.Result == 1) then
        ...
        self.kickedTimeId = self:subscibe_time_event(gameState.RestTime, false, 1):OnUpdate(function(t)
            ...
            ModuleCache.ModuleManager.hide_module("majiangshanxi", "matchdialog")
        end).id
    end

    if (4 == curTableData.RoomType and gameState.Result == 1) then --金币场小结算踢人倒计时
        if self.kickedTimeId then
            CSmartTimer:Kill(self.kickedTimeId)
        end
        self.kickedTimeId = self:subscibe_time_event(gameState.RestTime, false, 1):OnUpdate(function(t)
    ...
    if curTableData.RoomType == 3 and  gameState.RestTime ~= 1000 then
        ModuleCache.ModuleManager.show_module("henanmj","matchdialog",
        {
            btnType = 1,
...
function OneGameResultView:set_result(index)
    ...
    if self.ConfigData.isHuangShiHH and  not self.ConfigData.isDaYeKKF  then ---黄石晃晃麻将显示分数除以十显示
        BeiShu = BeiShu / 10
    end
    ...
    if self.goldMode then ---金币场特殊处理
        for i=1,#gameState.ScoreSettle do
            if gameState.ScoreSettle[i].SeatID == (index - 1) then
                BeiShu = gameState.ScoreSettle[i].Amount
                break
            end
        end
    end
...
function OneGameResultView:is_laizi(pai, newGameState)
    return pai ~= 0 and (newGameState.LaiZi == pai or (self.ConfigData.isWuZhou and newGameState.LaiGen == pai and curTableData.ruleJsonInfo.GuiPai == 2))
end
