...
--显示比赛场
	if(curTableData.RoomType == 3) then
		ModuleCache.ModuleManager.show_module(self.packageName, "tablematch")
	end
end
...
function TableModule:on_module_event_bind()
	...
	self:subscibe_model_event("Event_Msg_Exit_Room", function(eventHead, eventData)
		if(eventData.Error and eventData.Error == 0)then
			if ModuleCache.ModuleManager.module_is_active(self.packageName, "totalgameresult") then
...
function TableModule:on_click(obj, arg)	
	...
	if(obj == self.tableView.buttonSetting.gameObject) then		
		...
		intentData.majiangPackageName = 'majiangshanxi'
		ModuleCache.ModuleManager.show_module("henanmj", "roomsetting", intentData)
		-- 先保留，因为安庆有
		-- ModuleCache.ModuleManager.show_module("henanmj", "roomsetting", not self.view.inviteAndExit.activeSelf)
	elseif(obj == self.tableView.buttonWarning.gameObject) then
		...
		--ModuleCache.ModuleManager.show_module("henanmj", "tablegps", self.view.tipText .. "," .. self.view.distanceText)
	...
	elseif (obj.name == "ButtonChat") then
		...
		{is_New_Sever=self:is_new_version(), config = Config, backgroundStyle = "BackgroundStyle_1", chatShotTextList = self.view.ConfigData.chatShotTextList})
	...
	elseif (obj.name == "ButtonBackToSettle") then
		if self.view.ConfigData.isHuangShiHH then --现在只有黄石晃晃有这个需求
			ModuleCache.ModuleManager.show_module(self.packageName, "onegameresultpuning",self.presettlementState)
		end
	elseif (obj.name == "ButtonContinue") then
		if self.view.ConfigData.isHuangShiHH then --现在只有黄石晃晃有这个需求
			ModuleCache.ModuleManager.destroy_module(self.packageName, "onegameresultpuning")
...
function TableModule:show_game_result(newGameState)
    ...
    if(newGameState) then
		...
		elseif((showDissolve and newGameState.Result == 2) or (newGameState.Result == 2 and (not self.showOneResult ) )) then
			...
			ModuleCache.ModuleManager.show_module(self.packageName, "totalgameresult")
...
function TableModule:show_maima_custom(newGameState)
	...
	if # maiMa >0 then
		...
		self:subscibe_time_event(1, false, 0):OnComplete(function(t)
			if(self:can_show_result()) then
				ModuleCache.ModuleManager.show_module(self.packageName, "tablepop", newGameState)
			end
		end)
	end
	...
	self:subscibe_time_event(waitTime, false, 0):OnComplete(function(t)
		if(self:can_show_result()) then
			if(ModuleCache.ModuleManager.module_is_active(self.packageName, "tablepop")) then
				ModuleCache.ModuleManager.hide_module(self.packageName, "tablepop")
			end
			if self.view.ConfigData.pnShowResult then
				ModuleCache.ModuleManager.show_module(self.packageName, "onegameresultpuning", newGameState)
			else
				ModuleCache.ModuleManager.show_module(self.packageName, "onegameresult", newGameState)
			end
		end
	end)
end
...
function TableModule:show_table_presettlement(state)
	if 4 == curTableData.RoomType then
		...
		if self.gameState and self.gameState.Result == 2 then  --如果是大结算
			ModuleCache.ComponentUtil.SafeSetActive(self.view.buttonContinue,false)
...
function TableModule:can_show_result()
	local showTotalResult = ModuleCache.ModuleManager.module_is_active(self.packageName, "totalgameresult")
