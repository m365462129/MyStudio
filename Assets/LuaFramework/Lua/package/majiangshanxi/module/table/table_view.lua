...
function TableView:initialize(...)
    ...
    self.packageName = 'henanmj'
    self.moduleName = 'table'
    ViewBase.initialize(self, "majiangshanxi/module/table/henanmj_table.prefab", "HeNanMJ_Table", 0, true)
...
function TableView:init_config()
    ...
    elseif((self.ConfigData.isHuangShiHH or self.ConfigData.isDaYeKKF) and not self.ConfigData.isEnShiMJ) then
        mCPGHLClipName =
        {
            ["1"] = {"chi1","chi2"},
            ["2"] = {"peng1","peng2"},
    ...
    elseif (self.ConfigData.isEnShiMJ) then
        mCPGHLClipName["67"] = {"gang1"}
    end
...
function TableView:setRightMenuState(state)
    ...
    local closeEixtBtn = (4 == curTableData.RoomType)
...
function TableView:refresh_game_state(data, onlySet)
    ...
    if(data.Result == 0 and self.ConfigData.isKaWuXing and # data.MaiMa >0) then
        ...
        self:subscibe_time_event(2.5, false, 0):OnComplete(function(t)
            ModuleCache.ModuleManager.show_module("majiangshanxi", "tablepop", data)
        end)
    end
...
function TableView:refresh_timedown_clock()
    ...
    ComponentUtil.SafeSetActive(self.laiGenMJ1, gameState.LaiGen ~= 0 and self.ConfigData.isBaPiSiLai)
    ComponentUtil.SafeSetActive(self.laiZiMJ, gameState.LaiZi ~= 0)
    if(gameState.LaiGen ~= 0) then
        if self.ConfigData.isEnShiMJ and self.ConfigData.isBaPiSiLai then
            local grid = GetComponentWithPath(self.baoPai, "Grid", ComponentTypeName.Transform).gameObject
            grid.transform.localPosition = Vector3.New(-640, grid.transform.localPosition.y, grid.transform.localPosition.z)
...
function TableView:get_laigen(laiGen)
    if (laiGen%9 == 1) then
        return laiGen + 8
    else
        return laiGen - 1
...
function TableView:chu_pai_add_qi_zhang(pai, mj)
    ...
    if (self.ReadyTing) then
        table.insert(playerState.QiZhang, 0)
    else
...
function TableView:refresh_wait_action()
    ...
    -- if(#gameState.KeLiang > 0 and gameState.KeChu) then
    --     print("听牌判断11")
    --     ComponentUtil.SafeSetActive(self.waitAction, true)
    --     ComponentUtil.SafeSetActive(self.actionBtTing, true)
    --     ComponentUtil.SafeSetActive(self.actionBtGuo, true)
    -- end
...
function TableView:refresh_mj_color_scale(refreshState)
    local config = ModuleCache.PlayModeUtil.get_playmodel_data(curTableData.ruleJsonInfo.GameType)
