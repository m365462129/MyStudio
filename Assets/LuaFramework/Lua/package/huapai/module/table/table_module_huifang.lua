...
function TableModule:init_playback_data()
    ...
    if curTableData.isPlayBack then
        self.playbackModule = ModuleManager.show_module("huapai", "playback")
...
function TableModule:playback()
    ...
    if data.result == 2 then
        DataHuaPai.Msg_Table_GameStateNTF2 = data
    elseif data.result == 1 then
        DataHuaPai.Msg_Table_GameStateNTF1 = data
    end

    if data.result ~= 0 then
        self:show_game_result(data)
    else
        ModuleManager.hide_module("huapai", "singleresult")
    end
