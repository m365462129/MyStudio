...
function TableModule:refresh_report_state(data)
    ...
    if DataHuaPai.Msg_Table_GameStateNTF == nil and curTableData.RoomType == 2 and tonumber(data.State) == 2 and (not curTableData.CurRound or (curTableData.CurRound and curTableData.CurRound ==0))  then

        self.playersView[localSeatID]:show_kick(not (self:is_all_ready() and not DataHuaPai.Msg_Table_GameStateNTF ))
...
function TableModule:show_start_btn()
    ...
    if curTableData.RoomType == 2 then
        if DataHuaPai.Msg_Table_UserStateNTF_Self and not DataHuaPai.Msg_Table_UserStateNTF_Self.Ready then
            Manager.SetActive(self.view.btnStartZhunBei_museum.gameObject,(not DataHuaPai.Msg_Table_GameStateNTF or DataHuaPai.Msg_Table_GameStateNTF.RealyRound == 0) )
        end
    end
    ...
    if not DataHuaPai.Msg_Table_UserStateNTF_Self then
        return
    end
    ...
    if self.view.btnStartZhunBeiText then
        ...
        if DataHuaPai.Msg_Table_UserStateNTF_Self and DataHuaPai.Msg_Table_UserStateNTF_Self.Ready then
            Manager.SetActive(self.view.btnStartZhunBei.gameObject, false)
        else
        ...
        if (AppData.Game_Name == 'XXZP' or AppData.Game_Name == 'LDZP') then
            ...
            if DataHuaPai.Msg_Table_UserStateNTF_Self.PiaoNum == -1 and num == 3 and ruleInfo.DaTuo == 1 then
        ...
        if AppData.Game_Name == 'DYZP' then
            ...
            local data = DataHuaPai.Msg_Table_UserStateNTF
            local isFangZhu = data.ZhuangJia == curTableData.SeatID
            local num = DataHuaPai.Msg_Table_UserStateNTFNumPrep or 0
            if isFangZhu then
                if num == 2 then
...
function TableModule:refresh_user_state(data)
    ...
    for i = 1, #data.State do
        if not curTableData.isPlayBack then
            ...
            if DataHuaPai.Msg_ReportStateNTF_Table and DataHuaPai.Msg_ReportStateNTF_Table[data.State[i].SeatID] then
                self.playersView[localSeatID]:show_leave(DataHuaPai.Msg_ReportStateNTF_Table[data.State[i].SeatID].State == 1)
                self.playersView[localSeatID]:show_offline(DataHuaPai.Msg_ReportStateNTF_Table[data.State[i].SeatID].State == 2)
...
function TableModule:ShowGpsInfo(hide)
    ...
    self:start_lua_coroutine(function ()
        ...
        if hide then
            ...
            print(UnityEngine.PlayerPrefs.GetString(str))
            if tablelocation:isHavaZuoBiPlayer(seatHolderArray) and UnityEngine.PlayerPrefs.GetString(str) == "" then
