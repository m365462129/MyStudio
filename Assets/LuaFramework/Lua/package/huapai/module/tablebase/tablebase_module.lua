...
function TableBaseModule:on_module_event_bind()
    ...
    self:subscibe_model_event("Event_Msg_SameUserLoginNTF", function(eventHead, eventData)
        ...
        ModuleCache.ModuleManager.destroy_package("huapai")
    ...
    self:subscibe_model_event("Event_Msg_DismissNTF", function(eventHead, eventData)
        if (#eventData.Action == 0) then
            ...
            ModuleCache.ModuleManager.hide_module("huapai", "dissolveroom")
        else
            ModuleCache.ModuleManager.show_module("huapai", "dissolveroom", eventData)
        end
    end)
    ...
    self:subscibe_model_event("Event_Msg_Exit_Room", function(eventHead, eventData)
        ...
        if (eventData.Error and eventData.Error == 0) then
            self:exit_room()
            --if ModuleCache.ModuleManager.module_is_active("huapai", "totalresult") then
...
function TableBaseModule:update_seat_location(SeatID, data)
    if DataHuaPai.playersView and DataHuaPai.playersView[SeatID] then
        DataHuaPai.playersView[SeatID].playerInfo.locationData = data
    end
end
...
function TableBaseModule:on_send_gift_chat_msg(seatId, content)
    if(string.sub(content, 1, 1) == "{")then
        ...
        local sendPlayerView = DataHuaPai.playersView[self:get_local_seat(seatId)]
        local receiverPlayerView = DataHuaPai.playersView[self:get_local_seat(gift.receiver)]
