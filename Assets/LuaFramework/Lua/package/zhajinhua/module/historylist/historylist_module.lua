...
function HistoryListModule:on_click(obj, arg)
    ...
    elseif obj == self.historyListView.buttonLookMatch.gameObject then
        ...
        -- 点击战绩item模板
    elseif obj.transform.parent.gameObject == self.historyListView.itemContent.gameObject then
        -- 通过索引获取历史战绩信息
...
function HistoryListModule:getRoomList(roomInfo)
    ...
    self:http_get(requestData, function(wwwOperation)
        ...
        if retData.ret and retData.ret == 0 then
            ...
            retData.data.settleType=roomInfo.settleType;
...
function HistoryListModule:on_videoData(data)
    ...
    local intentData = {
        packageName = 'guandan',
        moduleName = 'guandan_table_video',
        videoData = data,
    }
    ModuleCache.ModuleManager.show_module("guandan", "guandan_table_video_player", intentData)
end
