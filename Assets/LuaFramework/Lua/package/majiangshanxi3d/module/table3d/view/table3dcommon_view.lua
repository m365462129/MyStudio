...
local ViewBase = require('package.majiangshanxi3d.module.table3d.table3d_view')
...
local Mj3dDun = require("package.majiangshanxi3d.module.table3d.Mj3dDun")
local Mj3dCenterCom = require("package.majiangshanxi3d.module.table3d.Mj3dCenterCom")
local SeatHolder3D =  require('package.majiangshanxi3d.module.table3d.seat.seatholder3d')
...
function Table3dCommonView:init_seats()
    ...
    ---@type Pool
    self.pool = require('package.majiangshanxi.module.tablenew.pool'):new(self.cloneParent)
end
---初始化单个座位
        ...
        Mj3dCenterCom:playDiceAni(self.gameState.Dice1,self.gameState.Dice2,function ()
            self:playDiceAndDealAni(self.gameState,function ()
                ...
                self:playDisLaiGen(self.gameState,function ()
                    ...
                    local LaiZis = self:get_laizi_list(self.gameState)
...
--- 计算癞子
function Table3dCommonView:get_laizi_list(gameState)
    return {self.gameState.LaiZi}
end
...
function Table3dCommonView:game_state_dun()
    ...
    local ZhuangJia = self.gameState.ZhuangJia
    local leftDunCount = dice1 >= dice2 and dice2 or dice1 ---留墩的数量
    local isPlayDelAni = true
    Mj3dDun:refreshDuns(Dun,TotalCount,DunStart,ZhuangJia,leftDunCount,isPlayDelAni)
end
...
function Table3dCommonView:playDiceAndDealAni(gameState,callBack)
    ...
    self:subscibe_time_event(dunAniTotalTime + 1.25, false, 0):OnComplete(function()
        if callBack then
            callBack()
        end
    end)
...
function Table3dCommonView:playShowDun(gameState,callback)
    ...
    for i=1,dunTotal do
        ...
        self:subscibe_time_event(0.05 * (i-1), false, 0):OnComplete(function()
            ---@type Mj3dThrowMj
            ...
            local realIndex = Mj3dDun:getLastDelDunMjIndex() - index
            if realIndex < 1 then
                realIndex = realIndex + gameState.TotalCount
            end
