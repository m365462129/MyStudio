...
local Mj3dKan = require("package.majiangshanxi3d.module.table3d.Mj3dKan")
local Mj3dHelper = require("package.majiangshanxi3d.module.table3d.table3d_helper")
local Mj3dThrowMj = require("package.majiangshanxi3d.module.table3d.Mj3dThrowMj")
...
function Mj3dHodler:Create(tableSeat,parent)
    ...
    if 1 == hodler.tableSeat.seatId then
        ...
        if 14 == hodler.tableSeat.view.maxMjNum then
            pos = Vector3.New(19.5,35.1,-51.6)
        elseif 20 == hodler.tableSeat.view.maxMjNum then
            pos = Vector3.New(21.8,35.1-49)
        end
        ...
        for i=1,#gameState.Action do
            ...
            if action.SeatID == serverIndex then
                if action.Action == 8 or  action.Action == 9 then
                    ...
                    self:displayMjOnScreen(pai,1,function ()
                    end)
                end
                break
            end
...
function Mj3dHodler:onHandMjDrag(obj)
    ...
    screenPos.z =  self.tableSeat.view.disOnScreenTrans.position.z - self.tableSeat.view.cam3d.transform.position.z
...
function Mj3dHodler:simulationOutMj(pai,callback)
    self:displayMjOnScreen(pai,1,nil)
