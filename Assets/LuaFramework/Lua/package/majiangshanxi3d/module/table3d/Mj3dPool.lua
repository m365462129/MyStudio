...
function Mj3dPool:Get(pai,parent)
    ...
    if not mjGo then
        local majiangTemplate = self.mjTemplateRoot:Find(resName).gameObject
...
function Mj3dPool:setMjType(type)
    self:clearCache()
    self.mjTemplateRoot = self.hodlerTrans:Find(tostring(type))
end
