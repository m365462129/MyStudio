...
function TotalResultModule:on_show(intentData)
    intentData = intentData or DataPaoHuZi.Msg_Table_GameStateNTF
    --- 加载时隐藏，手动调用显示函数
...
function TotalResultModule:show_result()
    ...
    if DataPaoHuZi.Msg_Table_UserStateNTF_Self and DataPaoHuZi.Msg_Table_UserStateNTF_Self.Ready == true and DataPaoHuZi.Msg_RoomDismissedNTF ~= nil then
        Manager.SetActive(self.view.root, false)
    end
...
function TotalResultModule:set_players(data)
    ...
    for i=1,10 do
        print(DataPaoHuZi.Msg_Table_UserStateNTF)
    end

    local retData = DataPaoHuZi.Msg_Table_UserStateNTF or {}
    ...
    for i = 1, #data.player do
        ...
        if AppData.Game_Name ~= 'GLZP' then
            ...
            local str1 = "<color=#7a3913>" .. data.player[i].total_jie_suan_hu_xi .. '</color>\n'
            str1 = str1 ..  "<color=#7a3913>" .. data.player[i].total_hu_xi .. '</color>\n'

            if not (AppData.Game_Name == 'XXZP' or AppData.Game_Name == 'LDZP') then
                str1 ="<color=#7a3913>" .. data.player[i].total_hu_xi .. '</color>\n'
                str1 = ""
            end
            for i=1,#datas do
...
function TotalResultModule:on_click(obj, arg)
    ...
    elseif obj == self.view.ButtonNext.gameObject then
        ...
        self:start_lua_coroutine(function ()
            ...
            DataPaoHuZi.NoJieXiGameState = true
            local selfGS = DataPaoHuZi.Msg_Table_UserStateNTF_Self
            ...
            for i=1,10 do
                ...
                if not self.view then
                    DataPaoHuZi.NoJieXiGameState = nil
                    return
                end
            end

            if DataPaoHuZi.Msg_Table_UserStateNTF_Self == nil then
                DataPaoHuZi.Msg_Table_UserStateNTF_Self = selfGS
            end
            ...
            DataPaoHuZi.NoJieXiGameState = nil
            if DataPaoHuZi.Msg_Table_UserStateNTF_Self.ErrCode == -11 or DataPaoHuZi.Msg_Table_UserStateNTF_Self.ErrCode == -23 then
            ...
            elseif DataPaoHuZi.Msg_Table_UserStateNTF_Self.ErrCode == 0 then
                ModuleCache.ModuleManager.destroy_module("paohuzi", "totalresult")
...
function TotalResultModule:setJieSanInfo()
    ...
    if DataPaoHuZi.Msg_DismissNTF == nil then
        return 
    end
    local freeRoomData = DataPaoHuZi.Msg_DismissNTF
...
function TotalResultModule:get_result_share_data()
    ...
    if(DataPaoHuZi.Msg_Table_GameStateNTF.endTime)then
        resultData.endTime = os.date("%Y/%m/%d %H:%M:%S", DataPaoHuZi.Msg_Table_GameStateNTF.endTime)
    end
