...
local ModuleBase = require("package.paohuzi.module.tablebase.tablebase_module")

---@class PaoHuZiTableModule:TableBaseModule
---@field view PaoHuZiTableView
---@field model PaoHuZiTableModel
local TableModule = class("PaoHuZi.TableModule", ModuleBase)
...
function TableModule:initialize(...)
    ...
    require("package.paohuzi.module.table.table_module_huifang")
    require("package.paohuzi.module.table.table_module_gameState")
    require("package.paohuzi.module.table.table_module_other")
    require("package.paohuzi.module.table.table_module_userState")
    ...
    self.view.btnLeave1.onClick:AddListener(
        function()
            if DataPaoHuZi.Msg_Table_GameStateNTF == nil then
...
function TableModule:bind_players_view()
    self.playersView = {}
    DataPaoHuZi.playersView = self.playersView
...
function TableModule:quxiaoTuoGuan(datat)
    ...
    if datat and DataPaoHuZi.Msg_Table_GameStateNTF.result == 0 then
...
function TableModule:set_jiang(value)
    if
        not value or value == 0 or
            (DataPaoHuZi.Msg_Table_GameStateNTF and DataPaoHuZi.Msg_Table_GameStateNTF.result == 1)
     then
        Manager.SetActive(self.view.jiangpaiObj, false)
    ...
    self.view.jiangpaiImg.sprite = TableUtilPaoHuZi.getcardSprite(value, "ZiPai_PlayCards")
...
function TableModule:on_module_event_bind()
    ...
    self:subscibe_module_event(
        ...
        function(eventHead, eventData)
            if DataPaoHuZi.Msg_Table_GameStateNTF == nil or DataPaoHuZi.Msg_Table_GameStateNTF.SeqNo > 3 then
                self:reset()
            end
    ...
    self:subscibe_module_event(
        ...
        "Event_Playback_Exit",
        function(eventHead, eventData)
            ModuleManager.destroy_module("paohuzi", "table")
...
function TableModule:refresh_round(round)
    ...
    self.view.txtJushu.text = string.format("第%d/%d局", round, curTableData.RoundCount)
    if (AppData.Game_Name == 'XXZP' or AppData.Game_Name == 'LDZP') then
        if DataPaoHuZi.Msg_Table_GameStateNTF then
            self.view.txtJushu.text = string.format("        第%d局", DataPaoHuZi.Msg_Table_GameStateNTF.RealyRound)
        else
            self.view.txtJushu.text = "       第1局"
        end
    end
end
...
function TableModule:invite_friend(shareToClipboard)
    ...
    if ruleInfo.baseScore and ruleInfo.isPrivateRoom ~= true then
        if not DataPaoHuZi.Msg_Table_GameStateNTF or DataPaoHuZi.Msg_Table_GameStateNTF.result ~= 0 then
            return
        end
    end
    ...
    if DataPaoHuZi.Msg_Table_UserStateNTF.BaseCoinScore then
        shareData.ruleName = string.format('金币场-匹配模式 底分:%d ', DataPaoHuZi.Msg_Table_UserStateNTF.BaseCoinScore)
    end

    if DataPaoHuZi.Msg_Table_UserStateNTF.BaseCoinScore == 0 then
...
function TableModule:getDangQianRenShu()
    return DataPaoHuZi.Msg_Table_UserStateNTFNumAll
end
...
function TableModule:show_settings()
    local intentData = {}
    intentData.tableBackgroundSpriteSetName = "RoomSetting_TableBackground_Name_" .. "PHZ"
    ...
    if AppData.Game_Name == "LDZP" or AppData.Game_Name == "XXZP" then
        intentData.isOpenLocationSetting = true
        intentData.defLocationSetting = 1
    end
...
function TableModule:on_destroy()
    ...
    ModuleManager.destroy_module("paohuzi", "playback")
    ModuleManager.destroy_module("paohuzi", "singleresult")
    ModuleManager.destroy_module("paohuzi", "totalresult")
