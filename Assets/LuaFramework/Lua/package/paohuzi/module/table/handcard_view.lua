...
function HandCardView:init_data(data, slowly, callback)
    local dataList = {}
...
function HandCardView:set_drag_img(value)
    TableUtilPaoHuZi.set_card(self.dragImg.transform.parent.gameObject, value, nil, "ZiPai_HandCards")
...
function HandCardView:on_drag_end()
    ...
    if self.downFlag and self.dragObj then
        ...
        --- 出牌范围
        if pos.y > self.lt.y then

            local enable_ke_chu = DataPaoHuZi.Msg_Table_GameStateNTF.ke_chu ~= 0
            --- 可出牌
            if
                enable_ke_chu and self.card[self.downX][self.downY].obj and
                    self.card[self.downX][self.downY].obj == self.dragObj
             then
                ...
                if not boolf then
                    ...
                    -- 直接刷新当前玩家 手牌 且播放 动画
                    if HandCardView.table_model then
                        HandCardView.table_model:selfChuPaiDongHua(value)
                    end      
                else
...
function HandCardView:fapai1(endPos)
    self.HandCardView_fapai1 = self.HandCardView_fapai1 or 1
    self.HandCardView_fapai1 = self.HandCardView_fapai1 + 1
    local num = self.HandCardView_fapai1 % 3 + 1
    local newObj = HandCardView.view.FaPai[1][num]
    newObj.transform.position = HandCardView.view.FaPai[1].position
    newObj.transform:DOMove(endPos, 0.1, false)
...
function HandCardView:fapai()
    local huxiObj = HandCardView.view.FaPaiTeXiao
    huxiObj.gameObject:SetActive(true)

    if AppData.Game_Name == 'DYZP' then
        Manager.SetActive(HandCardView.view.jiangpaiObj, false)
    end
    
    self.module:start_lua_coroutine(
        function()
            coroutine.wait(1)
            if self.module.view == nil then
                return
            end
            --UnityEngine.GameObject.Destroy(huxiObj)
            huxiObj.gameObject:SetActive(false)
            if AppData.Game_Name == 'DYZP' then
                Manager.SetActive(HandCardView.view.jiangpaiObj, true)
            end
        end
    )
end
