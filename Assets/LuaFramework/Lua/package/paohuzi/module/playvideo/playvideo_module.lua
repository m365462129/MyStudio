...
local PlayVideoModule = class("PaoHuZi.PlayVideoModule", ModuleBase)
...
local PlaybackDataManager = require("package.paohuzi.module.tablebase.playbackdata_manager")
...
function PlayVideoModule:on_click(obj, arg)	
	...
	if obj == self.playVideoView.buttonClose.gameObject then
		ModuleCache.ModuleManager.hide_module("paohuzi", "playvideo")
		return
...
function  PlayVideoModule:playVideo(strVideoId)
	if(strVideoId == "") then
		...
		PlaybackDataManager:play_back_on_id(strVideoId, self.onFinishGetVideoDataCallback)
	end
