...
local TableBiJiHelper = {}
local ModuleCache = ModuleCache
local GetComponentWithPath = ModuleCache.ComponentManager.GetComponentWithPath
local Sequence = DG.Tweening.DOTween.Sequence

function TableBiJiHelper:setSeatPlayerInfo(seatHolder, playerInfo)

end

function TableBiJiHelper:initDealTable(root)
    self.inhandPokers = {}
    self.matching = {};
    self.animMatching = {};
    TableBiJiHelper:initInHandPokers(root);
    for i = 1, 3 do
        TableBiJiHelper:initMatching(root,i);
    end
    TableBiJiHelper:initAnimMatching(root);
end

function TableBiJiHelper:initInHandPokers(root)
    for i = 1,9 do
...
function TableBiJiHelper:initMatching(root,index)
    self.matching[index] = {};
    local path;
    if(index == 1) then
...
function TableBiJiHelper:initAnimMatching(root)
    for i = 1,3 do
...
function TableBiJiHelper:initSeatHolder(seatHolder, seatIndex, goSeat, handPokerRoot)
    local root = goSeat
...
function TableBiJiHelper:initCardHolder(cardHolder, pokerAssetHolder)
...
function TableBiJiHelper:PlayAnimHandToMatch(handIndex,matchIndex,detailIndex,onFinishInvoke)
...
function TableBiJiHelper:PlayAnimMatchToMatch(srcIndex,desIndex,onFinish)
    local duration = 0.5;
...
function TableBiJiHelper:refreshInHandCards(seatHolder, inHandPokerList, showFace, useAnim)
...
function TableBiJiHelper:playCardTurnAnim(cardHolder, toFace, duration, delayTime, onFinish)
...
function TableBiJiHelper:playCardFlyToPosAnim(cardHolder, targetPos, duration, delayTime, onFinish)
    local sequence = self.module:create_sequence()
...
function TableBiJiHelper:playCardScaleAnim(cardHolder, targetScale, duration, delayTime, onFinish)
    local sequence = self.module:create_sequence()
...
--显示牛几
function TableBiJiHelper:showNiuName(seatHolder, show, niuName)

end



function TableBiJiHelper:setInHandPokersDonePos(seatHolder)
...
function TableBiJiHelper:setInHandPokersOriginalPos(seatHolder)
    seatHolder.handPokerCardsRoot.transform.position = seatHolder.inHandCardsOriginalPos
end


--播放随机选庄动画
function TableBiJiHelper:playRandomBankerAnim(seatHolderList, targetSeat, onFinish)
...
--播放设置庄家动画
function TableBiJiHelper:playSetTargetSeatAsBanker(seatHolder, onFinish)
...
--刷新座位相关信息
function TableBiJiHelper:refreshSeatInfo(seatHolder, seatData)
    if(seatData.playerId and (seatData.playerId ~= 0 and seatData.playerId ~= "0")) then        
        ...
        ModuleCache.ComponentUtil.SafeSetActive(seatHolder.imageCreator.gameObject, tonumber(seatData.playerId) == tonumber(self.modelData.curTableData.roomInfo.roomHostID)
        and (self.modelData.curTableData.roomInfo.curRoundNum == 0))
        ...
        ModuleCache.ComponentUtil.SafeSetActive(seatHolder.buttonKick.gameObject, seatData.canBeKicked and seatHolder.gameCount == 0 )
...
function TableBiJiHelper:refreshSeatOfflineState(seatHolder, seatData)
    ModuleCache.ComponentUtil.SafeSetActive(seatHolder.imageDisconnect.gameObject, seatData.isOffline)

    if self.modelData.roleData.RoomType == 2 and seatData.gameCount == 0 then--亲友圈快速组局
        if tonumber(self.modelData.roleData.userID) == tonumber(self.modelData.curTableData.roomInfo.roomHostID)
        and tonumber(self.modelData.curTableData.roomInfo.roomHostID) ~= tonumber(seatData.playerId) then
...
--刷新座位的状态
function TableBiJiHelper:refreshSeatState(seatHolder, seatData)
    if(seatData.playerId and (seatData.playerId ~= 0 and seatData.playerId ~= "0")) then        
...
function TableBiJiHelper:showSeatWinScoreCurRound(seatHolder, show, score)
...
function TableBiJiHelper:refreshClock(seatHolder, show, targetTime, curTime)
...
--显示或隐藏说话icon
function TableBiJiHelper:showSpeakIcon(seatHolder, show)
    ModuleCache.ComponentUtil.SafeSetActive(seatHolder.goSpeakIcon, show) 
end

--显示下注按钮
function TableBiJiHelper:setBetScore(seatHolder, seatData, show)    
...
--填写玩家信息
function TableBiJiHelper:setPlayerInfo(seatHolder, playerData)
...
--下载头像
function TableBiJiHelper:startDownLoadHeadIcon(targetImage, url, callback)    
...
--设置牌的数据
function TableBiJiHelper:setCardInfo(cardHolder, poker)    
...
--显示或隐藏牌
function TableBiJiHelper:showCard(cardHolder, show)
    ModuleCache.ComponentUtil.SafeSetActive(cardHolder.cardRoot, show)
end

--显示牌正面
function TableBiJiHelper:showCardFace(cardHolder)
...
--显示牌背面
function TableBiJiHelper:showCardBack(cardHolder)
...
--显示或隐藏手牌
function TableBiJiHelper:showInHandCards(seatHolder, show)    
    ModuleCache.ComponentUtil.SafeSetActive(seatHolder.handPokerCardsRoot, show)
end

function TableBiJiHelper:getImageNameFromPoker(poker)
...
function TableBiJiHelper:getImageNameFromNiuName(niuName)
    print(niuName)
    return niuName
end


function TableBiJiHelper:getSoundNameFromNiuName(niuName)
    local headStr = "cow_"
...
function TableBiJiHelper:getNumberFormPoker(poker)
    if(poker.number == "A") then
        return 1
    elseif (poker.number == "J") then 
        return 10
...
function TableBiJiHelper:checkHasNiuFormPokerArray(pokerArray, result)
    result = result or {}
...
function TableBiJiHelper:getSeatInfoByRemoteSeatIndex(remoteSeatIndex, seatInfoList)	
...
--将服务器的做座位索引转换为本地位置索引
function TableBiJiHelper:getLocalIndexFromRemoteSeatIndex(seatIndex, mySeatIndex, seatCount)
...
--判断是否所有入座的玩家都已准备
function TableBiJiHelper:checkIsAllReady(seatInfoList)
...
--通过玩家id获取座位信息
function TableBiJiHelper:getSeatInfoByPlayerId(playerId, seatInfoList)	
...
function TableBiJiHelper:getSeatedSeatCount(seatInfoList)
	local count = 0    
	for i=1,#seatInfoList do
...
function TableBiJiHelper:get_userinfo(playerId, callback)             
...
--播放牛名
function TableBiJiHelper:playNiuNameSound(niuName, isFemale)
...
function TableBiJiHelper:showNiuNiuEffect(seatHolder, show, duration, stayTime, delayTime, onComplete)
...
function TableBiJiHelper:showRoundScoreEffect(seatHolder, localSeatIndex, show, score, duration, delayTime, stayTime, onComplete)
...
function TableBiJiHelper:showRandomBankerEffect(seatHolder, show)
    ModuleCache.ComponentUtil.SafeSetActive(seatHolder.animSelectBanker.gameObject, show)    
end


function TableBiJiHelper:formatRuleDesc(rule)
    local desc = ""
