...
---@class TableBiJiModel : Model
local TableBiJiModel = class('tableBiJiModel', Model);

function TableBiJiModel:initialize( ... )
...
function TableBiJiModel:request_kick_player(player_id)
...
function TableBiJiModel:request_ready(isReady,userID)
...
function TableBiJiModel:request_complete_match(pokers , userID)
...
function TableBiJiModel:request_ping()
...
function TableBiJiModel:request_surrender(userID)
    local msgId, request = self.netMsgApi:create_request_data("Msg_Player_Surrender");
    request.userID = tonumber(userID);
    Model.send_msg(self, msgId, request); 
end

function TableBiJiModel:request_temporary_leave(isLeave, userID)
...
function TableBiJiModel:request_start()
    local msgId, request = self.netMsgApi:create_request_data("Msg_Table_Start")
    
    Model.send_msg(self, msgId, request) 
end

function TableBiJiModel:request_submit(pokers , userID)
...
--解散房间请求
function TableBiJiModel:request_dissolve_room(agree)
    local msgId, request = self.netMsgApi:create_request_data("Msg_Table_Dissolve_Room")
    request.agree = agree
    Model.send_msg(self, msgId, request)
end

function TableBiJiModel:request_exit_room(userID)
...
function TableBiJiModel:request_chat(msgType, text)
...
------位置信息变化请求
function TableBiJiModel:request_CustomInfoChange(customInfoJsonString)
