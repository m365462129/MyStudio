...
function TableRunfastModel:initialize( ... )
    ...
    Model.subscibe_msg_event(self, {   
        ...
        callback = function(msgName, msgBuffer)
            if msgBuffer.msgRetCode == 0 then
                print("=====Msg_Table_AgoSettleAccounts_Notify")
                local retData, error = Model.unpack_msg(self, msgName, msgBuffer.dataBuffer)                
    ...
    Model.subscibe_msg_event(self, {   
        ...
        callback = function(msgName, msgBuffer)
            if msgBuffer.msgRetCode == 0 then
                print("=====Msg_Table_ComputePoker_Notify")
                local retData, error = Model.unpack_msg(self, msgName, msgBuffer.dataBuffer)                
    ...
    Model.subscibe_msg_event(self, {   
        ...
        callback = function(msgName, msgBuffer)
            if msgBuffer.msgRetCode == 0 then
                print("=====Msg_Table_SettleAccounts_Notify")
                local retData, error = Model.unpack_msg(self, msgName, msgBuffer.dataBuffer)                
    ...
    Model.subscibe_msg_event(self, {   
        ...
        callback = function(msgName, msgBuffer)
            if msgBuffer.msgRetCode == 0 then
                print("=====Msg_Table_LastSettleAccounts_Notify")
                local retData, error = Model.unpack_msg(self, msgName, msgBuffer.dataBuffer)                
    ...
    Model.subscibe_msg_event(self, {   
        msgName = "Msg_Table_PlayerInfoNotify",
        callback = function(msgName, msgBuffer)
            if msgBuffer.msgRetCode == 0 then
                local retData, error = Model.unpack_msg(self, msgName, msgBuffer.dataBuffer)                         
                Model.dispatch_event(self, "Event_Table_PlayerInfoNotify", retData)                               
            end
        end
    })

    Model.subscibe_msg_event(self, {   
        msgName = "Msg_Table_PlayerCardReport",
        callback = function(msgName, msgBuffer)
            if msgBuffer.msgRetCode == 0 then
                local retData, error = Model.unpack_msg(self, msgName, msgBuffer.dataBuffer)                         
                Model.dispatch_event(self, "Event_Table_PlayerCardReport", retData)                               
            end
        end
    })
...
function TableRunfastModel:request_bet(betType)
    local msgId, request = self.netMsgApi:create_request_data("Msg_Table_Bet")
    request.bet_type = betType;				--倍数选择 1:2倍  2:3倍 3:4倍 4:10倍 
    --print("bet request :".. betType)
    Model.send_msg(self, msgId, request)
end

function TableRunfastModel:request_compute_poker()
