...
function CardPattern:compable(card_obj)
	...
	if self.type == CardCommon.type_four or card_obj.type == CardCommon.type_four
		then
		return true
	elseif self.card_cnt ~= card_obj.card_cnt
		then
		return false
	else
...
function CardPattern:parse(cards, owner_set_cnt, is_black3_player)
	...
	if (#card_type_stat[CardCommon.type_four] > 0)then
        ...
        CardCommon.SortAsc(card_type_stat[CardCommon.type_three])
	end
