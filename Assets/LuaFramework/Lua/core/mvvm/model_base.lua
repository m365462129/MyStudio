...
function Model:subscibe_msg_event(subscibeData, netClientName)
    ...
    subscibeData.msgId = self.netMsgApi:get_msg_data(subscibeData.msgName)[3]
...
function Model:unpack_msg(msgName, dataBuffer)
    ...
    if ModuleCache.GameManager.developmentMode then
        if GameConfigProject.netTransferDataShow and msgName ~= self.heartbeatResponseName then
            if tostring(msgName) == "9" or tostring(msgName) == "10" then
            else
                print_pbc_table(ret, "【" .. self.sendMsgNetClientName ..  "】收到消息包：" .. msgName)
            end
        end
        --记录日志
        if tostring(msgName) == "9" or tostring(msgName) == "10" or msgName == self.heartbeatResponseName then
        else
            -- ModuleCache.LogManager.custom_Log(log.format_pbc_table(ret, "【" .. self.sendMsgNetClientName ..  "】收到消息包：" .. msgName))
        end
    end
...
function Model:send_msg(msgName, requestData, netClientName, sendMsgHeadUseData)
    if ModuleCache.GameManager.developmentMode then
        ...
        --记录日志
        if tostring(msgName) == "9" or tostring(msgName) == "10" or msgName == self.heartbeatRequestName then
        else
            -- ModuleCache.LogManager.custom_Log(log.format_pbc_table(requestData, "【" .. (netClientName or self.sendMsgNetClientName) ..  "】请求发送消息包：" .. msgName))
        end
    end
    ...
    if client then
        ...
        --记录日志
        --ModuleCache.LogManager.custom_Log(log.format_pbc_table(requestData, "【" .. (netClientName or self.sendMsgNetClientName) ..  "】发送消息包失败，client为nil：" .. msgName))
    end
end
