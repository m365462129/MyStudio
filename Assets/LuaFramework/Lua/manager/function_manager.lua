...
--- 录音sdk
local GVoice = require("manager.gvoice_manager")
...
function Manager.GVoiceInit(id, key, userId, timeOut)
    if not userId then
        Manager.ShowTextPrompt("语音SDK初始化失败，userId = " .. userId)
        return
    end

    local gameId = tostring(id) or "1009944782"
    local gameKey = tostring(key) or "774efff9a827a9ed851bce91b8c4c876"
    local time = tonumber(timeOut) or 5000
    GVoice.init(gameId, gameKey, userId, time)
end

--- 设置语音SDK回调函数
function Manager.GVoiceSetCallback(uploadComplete, downloadComplete, playComplete)
    GVoice.set_event_listener(
    function(arg)
        if uploadComplete and "function" == Manager.Type(uploadComplete) then uploadComplete(arg) end
    end,
    function(arg)
        if downloadComplete and "function" == Manager.Type(downloadComplete) then downloadComplete(arg) end
    end,
    function(arg)
        if playComplete and "function" == Manager.Type(playComplete) then playComplete(arg) end
    end)
end

--- 清除语音SDK回调
function Manager.GVoiceClearCallback()
    GVoice.clear_event_listener()
end

--- 播放语音
---@param path string
function Manager.GVoicePlay(path)
    GVoice.play_recorded_file(path)
end
...
---@param path string
---@param c number
function Manager.GVoiceDownload(field, path, c)
    GVoice.download_recorded_file(field, path, c)
end

--- 上传录音
---@param path string
---@param c number
function Manager.GVoiceUpload(path, c)
    GVoice.upload_recorded_file(path, c)
end

--- 开始录音
---@param path string
function Manager.GVoiceStartRecording(path)
    GVoice.start_recording(path)
end

--- 停止录音
function Manager.GVoiceStopRecording()
    GVoice.stop_recording()
end

--- 停止播放
function Manager.GVoiceStopPlay()
    GVoice.stop_play_file()
end
--------------------------------------------------------
---  end  --- 录音相关函数
--------------------------------------------------------
